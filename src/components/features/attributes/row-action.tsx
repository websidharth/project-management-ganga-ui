...
interface AttributeRowActionsProps<TData> {
  row: Row<TData>;
  editRecord: (id: number) => void;
  deleteRecord: (id: number) => void;
}

export default function AttributeRowActions<TData>({ row, editRecord, deleteRecord }: AttributeRowActionsProps<TData>) {
  const item = row.original as AttributeDto;
